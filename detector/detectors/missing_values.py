...
class MissingValuesDetector(BaseDetector):
    ...
    def _build_fix_options(self, series, missing_mask, col_name) -> List[FixOption]:
        ...
        if is_numeric:
            ...
            name_hints = {"name", "customer", "person", "user", "client", "contact"}
            is_name_col = any(h in col_name.lower() for h in name_hints)

            mode_vals = clean.astype(str).mode()
            mode = mode_vals[0] if len(mode_vals) > 0 else None
